    ...
    private suspend fun loadData() {
        ...
        try {
            ...
            for (classroom in classrooms) {
                val tableRow = TableRow(this@CabinetActivity)

                val typeClassroomIdName = classroom.typeRoomId.toString()

                val cabinetName = TextView(this@CabinetActivity).apply {
                    text = typeClassroomIdName
                    setPadding(8, 8, 8, 8)
                }

                tableRow.addView(cabinetName)

                val currentCabinetLessons = paras.filter { it.classroomId == classroom.id }
                for (currentLesson in currentCabinetLessons) {
                    var strToAdd = ""
                    ...
                    if (currentLesson != null) {
                        ...
                        if (selectedDateString != formattedlessonDateString) {
                            val lessonDetails = TextView(this@CabinetActivity).apply {
                                text = ""
                                setPadding(8, 8, 8, 8)
                            }
