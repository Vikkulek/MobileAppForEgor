    ...
    private suspend fun loadData() {
        ...
        try {
            ...
            for (teacher in teachers) {
                val tableRow = TableRow(this@TeacherActivity)

                val teacherName = TextView(this@TeacherActivity).apply {
                    text = "${teacher.surname} ${teacher.name} ${teacher.patronymic}"
                    setPadding(8, 8, 8, 8)
                }

                tableRow.addView(teacherName)

                val currentTeacherLessons = paras.filter { it.teacherId == teacher.id }
                for (currentLesson in currentTeacherLessons) {
                    //val currentLesson = currentTeacherLessons.find { it.lessonNumber == i }
                    var strToAdd = ""
                    ...
                    if (currentLesson != null) {
                        ...
                        if (selectedDateString != formattedlessonDateString) {
                            val lessonDetails = TextView(this@TeacherActivity).apply {
                                text = ""
                                setPadding(8, 8, 8, 8)
                            }
                        ...
                        val group = groups.find { it.id == currentLesson.groupId }

                        strToAdd = "${subject?.name}\n" +
                                "Кабинет: ${classroom?.id}, ${typeRoom?.name}\n" +
                                "Группа: ${group?.name}"
                    }
